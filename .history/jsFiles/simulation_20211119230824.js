window.onload = () => {
    ...
    const controleChamps = () => {
        if (host.value === "" || temps.value === "") {
            ...
            console.log("La taillle : ", host.value.length)

            if (sliceValueHost === "http://") {
                if (host.value.length <= 18) {
                    bool = false;
    ...
    var nom = document.querySelector('#host');


    if (nom.value.length > 0 ) {
        let noms = statutValueSelected;
        let cible = nom.value;
        let sec = temps.value;
        console.log("Bool : ", host.value);
        console.log('Le nom, host, temps : ', nom, cible, sec)
