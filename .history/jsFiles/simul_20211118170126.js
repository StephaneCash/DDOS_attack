window.onload = () => {
    ...
    const controleChamps = () => {
        ...
        else if (caracSpe.test(valueHost) === false) {
            errHost.innerHTML = "<i> L'adresse doit contenir ce caractère spécial ' : ' </i>";
            errHost.style.color = "red"
            bool = false;
        } else if (number.test(valueHost) === false) {
            errHost.innerText = "L'adresse doit contenir des chiffres";
            bool = false;
        }
        else {
